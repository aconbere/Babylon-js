...
Babylon.Runner.prototype.call_on_observers = function(event, status, error){
    var that = this;

    Babylon.log.debug("Calling observers of " + event);
    this.observer.call_on_observers(event, function(observer, _){
        that.router.execute_route(observer, event, {status: status, error: error});
    });
};
