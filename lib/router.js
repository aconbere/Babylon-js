...
Babylon.Router.prototype.route = function(stanza) {
    ...
    if(routes.length >= 1){
        ...
        var route = routes[0];
        Babylon.log.debug("routing from query: " + route.query + " to: " route.controller.name + ", " + route.action);
...
Babylon.Router.prototype.raise = function(name, args) {
    ...
    if(events && events.length > 0) {
        for (var i = 0; i < events.length; i++) {
            var e = events[i];
            Babylon.log.debug("routing from event: " + name + " to : " + e.controller.name + ", " + e.action);
