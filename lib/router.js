...
Babylon.Router.prototype.execute_route = function(controller, action, stanza) {
    Babylon.log.debug("executing route: " + controller.prototype.name + ", " + action);
    s = Babylon.Stanza.wrap(controller.prototype.name, action, stanza);
    Babylon.log.debug(s);
    var c  = new controller(s)
    Babylon.log.debug("passed controller");
    c.perform(action);
    var v = c.evaluate()
