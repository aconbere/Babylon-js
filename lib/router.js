...
Babylon.Router.prototype.route = function(stanza) {                                                                                                          
  ...
  if ($.browser.msie) {
    var xmlDoc = Strophe._makeGenerator();
    xmlDoc.loadXML("<div>"+ Strophe.serialize(stanza) + "</div>");
    wrapped_stanza = $(xmlDoc.documentElement); 
  } else {
  ...
  //Babylon.log.debug("Wrapped stanza: " + wrapped_stanza);
